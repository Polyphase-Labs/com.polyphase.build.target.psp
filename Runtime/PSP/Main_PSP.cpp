...
// Folder under ms0:/PSP/GAME/ that holds this build's EBOOT.PBP. Normally
// injected by Makefile_PSP from PSP_GAME_DIR=<projectName> (which the PSP
// addon's GetCompileCommand sets from ctx->projectName). The POLYPHASE
// fallback only fires for manual builds that don't pass PSP_GAME_DIR.
// Folded into the boot-log paths below via literal-string concatenation
// so the runtime writes to ms0:/PSP/GAME/<projectName>/polyphase.log.
...
//
// Path: ms0:/PSP/GAME/<PSP_GAME_DIR>/polyphase.log
//   On PPSSPP that's <user>/Documents/PPSSPP/memstick/PSP/GAME/<PSP_GAME_DIR>/polyphase.log
...
static void Psp_BootLog(const char* msg)
{
    ...
    const SceUID fd = sceIoOpen("ms0:/PSP/GAME/" POLYPHASE_PSP_GAME_DIR "/polyphase.log",
